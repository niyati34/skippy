...
import { Calendar, BookOpen, Target, Gamepad2, Menu, MessageCircle, FileText } from 'lucide-react';
          ...
          <SheetContent side="left" className="w-80">
            <div className="flex flex-col h-full">
              <div className="mb-6">
                ...
                <p className="text-sm text-muted-foreground">Choose your study assistant</p>
              </div>
            ...
            <div className="flex flex-col flex-1 pt-5 pb-4 overflow-y-auto">
              <div className="flex items-center flex-shrink-0 px-4 mb-8">
                <div className="text-center w-full">
                  <h1 className="text-xl font-bold cyber-text-glow">Study Dashboard 🎓</h1>
                  <p className="text-sm text-muted-foreground mt-1">Let's make learning fun, {userName}!</p>
                </div>
              </div>
        ...
        <div className="flex flex-col flex-1 md:pl-64">
          <main className="flex-1">
            <div className="py-6 px-4 sm:px-6 lg:px-8">
              {activeTab === 'ai-chat' && (
                <Card className="h-[calc(100vh-8rem)] cyber-glow">
                  <CardHeader className="pb-4">
                    ...
                    <CardDescription>
                      Chat with Skippy to organize your studies, upload files, and get instant help
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="h-[calc(100%-6rem)] p-0">
                    <DashboardAI 
                      onScheduleUpdate={(items) => setScheduleItems(prev => [...prev, ...items])}
                      onFlashcardsUpdate={(cards) => setFlashcards(prev => [...prev, ...cards])}
                      onFunLearningUpdate={(content, type) => setFunLearningContent(content)}
                      onNotesUpdate={(newNotes) => setNotes(prev => [...prev, ...newNotes])}
                    />
                  </CardContent>
                </Card>
              )}
              ...
              {activeTab === 'schedule' && (
                <ScheduleChart scheduleItems={scheduleItems} />
              )}
