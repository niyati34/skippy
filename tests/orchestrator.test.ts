...
import { Orchestrator, NotesAgent, PlannerAgent, FlashcardAgent, FunAgent, BuddyAgent } from '@/lib/agent';

// Mock services used by agents
vi.mock('@/services/openrouter', () => ({
  callOpenRouter: vi.fn(async () => 'ok'),
  generateNotesFromContent: vi.fn(async (text: string) => [{ title: 'T', content: 'C', category: 'General', tags: [] }]),
  generateScheduleFromContent: vi.fn(async (text: string) => [{ id: '1', title: 'Event', date: '2025-01-01', time: '09:00' }]),
  generateFlashcards: vi.fn(async (text: string) => [{ question: 'Q', answer: 'A', category: 'General' }]),
  generateFunLearning: vi.fn(async (text: string, kind: string) => `${kind}:${text.slice(0,10)}`),
}));
