...
import { Orchestrator, BuddyAgent, CommandAgent, NotesAgent, PlannerAgent, FlashcardAgent, FunAgent } from './src/lib/agent.ts';
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      if (testCase.expectsScheduleCheck) {
        if (result.summary.includes("schedule") || result.summary.includes("nothing") || result.summary.includes("item")) {
      ...
      if (testCase.expectsScheduleCreation) {
        if (result.artifacts?.schedule || result.summary.includes("schedule") || result.summary.includes("created")) {
