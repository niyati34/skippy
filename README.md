...
<!--
Short Description: AI-powered study dashboard with weekly timetable, smart notes, flashcards, and a voice assistant. Built with React, Vite, Tailwind, OpenRouter, and Vercel.
Website: https://skippy-kohl.vercel.app
Topics: study dashboard ai timetable notes flashcards react vite tailwind openrouter vercel assistant
-->

# Skippy — AI Study Dashboard with Weekly Timetable, Notes, and Voice Assistant

Organize classes, generate smart study notes and flashcards, and chat with a friendly voice assistant. Built with React + Vite + Tailwind + shadcn/ui. AI calls go through a serverless proxy using OpenRouter.
...
- AI-Powered Notes & Flashcards
  - Uses OpenRouter for structured markdown with headings, key points, and summaries
...
- Zero-config Production API
  - Vercel serverless function: `api/openrouter/chat.js`
...
# Optional: start the local OpenRouter proxy (Express)
npm run server
...
You can use the local proxy. In production, the proxy is always used.
...
- Vercel function: `api/openrouter/chat.js`
...
- AI calls go to `/api/openrouter/chat` (OpenRouter).
- Serverless function is configured in `vercel.json`.
